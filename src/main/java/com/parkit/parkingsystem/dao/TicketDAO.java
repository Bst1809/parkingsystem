...
public class TicketDAO {
	...
	public int getRegCount(String vehicleRegNumber) { // Get the number of times a vehicle registered in the past
		...
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.GET_REGISTRATION_COUNT);
			ps.setString(1, vehicleRegNumber);
			ResultSet rs = ps.executeQuery();
			rs.next();
			regCount = rs.getInt(1);
			// if (regCount > 1) {
			// logger.info("Welcome back! As a recurring user of our parking lot, you'll
			// benefit from a 5% discount.");
			// }
			// System.out.println(
